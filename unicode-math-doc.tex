...
examples: \cmd\symbf\ in the former (`$\symbfup{M}$'), and \cmd\bm\ (or
...
Therefore, |\symsf{\alpha}| does not make sense (it produces `$\symsf{\alpha}$'), while |\symbfsf{\alpha}| gives `$\symsf{\alpha}$' as expected.
...
\begin{verbatim}
  \setmathfont{xits-math.otf}[range={\symcal,\symbfcal},StylisticSet=1]
\end{verbatim}
An example is shown below.
\begin{quote}
\setmathfont{xits-math.otf}[range=\symscr]
\setmathfont{xits-math.otf}[range=\symcal,StylisticSet=1]
