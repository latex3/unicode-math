...
\item |[range=\mathsfit->\mathbfsfit]| to map to different output styles(s) (which is rather useless right now but will become less useless in the future).
\end{itemize}
...
\paragraph{Operator font}
On a related note, \LaTeX\ defines an internal command \cs{operator@font} for typesetting elements such as |\sin| and |\cos|.
...
\subsection{Default `text math' fonts}

The five `text math' fonts, discussed above, are: \cs{mathrm}, \cs{mathbf}, \cs{mathit}, \cs{mathsf}, and \cs{mathtt}.
These commands are also defined under the synonyms: \cs{mathtextrm}, \cs{mathtextbf}, and so on.
...
\paragraph{Replacing `text math' fonts by symbols}
